...
<body class="bg-slate-100 min-h-screen">
    ...
    <div class="max-w-2xl mx-auto px-4 py-8">
        ...
        <div class="mb-6 flex items-center gap-3">
            ...
            <h2 class="text-xl font-bold text-slate-800">Tambah Tutorial</h2>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm p-8">
            ...
            <form method="POST" action="{{ route('tutorials.store') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <select name="kode_matkul"
                        ...
                        @foreach ($makulList as $makul)
                            <option value="{{ $makul['kode'] ?? $makul }}"
                                {{ old('kode_matkul') === ($makul['kode'] ?? $makul) ? 'selected' : '' }}>
                                {{ $makul['nama'] ?? $makul }} ({{ $makul['kode'] ?? $makul }})
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="url_presentation" value="{{ old('url_presentation') }}"
                           placeholder="https://..."
                           class="w-full px-3.5 py-2.5 border rounded-lg text-sm text-slate-800
                                  placeholder-slate-400 focus:outline-none focus:ring-2
                                  focus:ring-blue-500 focus:border-transparent
                ...
                <div>
                    ...
                    <input type="email" name="creator_email" value="{{ old('creator_email') }}"
