...
<body class="bg-t_navy min-h-screen">
    ...
    <div class="max-w-2xl mx-auto px-4 py-8">
        <div class="mb-6 flex items-center gap-3">
            ...
            <div>
                <h2 class="text-xl font-bold text-white">Edit Detail Tutorial</h2>
        ...
        <div class="bg-white rounded-xl shadow-2xl p-8 border-t-4 border-t_gold">
            ...
            <form method="POST" action="{{ route('tutorials.details.update', [$tutorial, $detail]) }}" enctype="multipart/form-data" class="space-y-5">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-bold text-t_navy mb-1">Order</label>
                        <input type="number" name="order" value="{{ old('order', $detail->order) }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-t_gold"/>
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <textarea name="text" rows="4" class="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-t_gold">{{ old('text', $detail->text) }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-bold text-t_navy mb-1">Gambar <span class="text-gray-400 font-normal">(opsional)</span></label>
                    @if ($detail->gambar)
                        <div class="mb-3 flex items-center gap-3 p-3 bg-gray-50 border border-gray-200 rounded-lg">
                            <img src="{{ Storage::disk('public')->url($detail->gambar) }}" class="w-16 h-16 object-cover rounded-lg border border-gray-300"/>
                        </div>
                    @endif
                ...
                <div>
                    ...
                    <textarea name="code" rows="5" class="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-t_gold">{{ old('code', $detail->code) }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-bold text-t_navy mb-1">URL <span class="text-gray-400 font-normal">(opsional)</span></label>
                    <input type="text" name="url" value="{{ old('url', $detail->url) }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-t_gold"/>
                </div>
